    ...
    try:
        ...
        if gmail_result.get('success') and gmail_result.get('is_gmail_query'):
            if gmail_result.get('requires_auth'):
                ...
                if auth_span:
                    auth_span.end(output={
                        "auth_required": True,
                        "auth_url": "/api/gmail/auth",
                        "message_type": "authentication_prompt"
                    })
                
            else:
                # SPAN 3B: Gmail API Request and Response
                if trace:
                    gmail_api_span = trace.span(
                        name="gmail_api_request_response",
                        input={
                            "intent": gmail_result.get('intent'),
                            "authenticated": True
                        }
                ...
                if gmail_data_result.get('success') and gmail_data_result.get('data'):
                    ...
                    gmail_api_span.update(output={
                        ...
                        "emails_preview": [{"from": email.get("from", "Unknown"), "subject": email.get("subject", "No Subject")[:50]} 
            ...
            if is_send_command and request.session_id in pending_post_packages:
                ...
                needs_approval = False
                hybrid_ai_span.update(output={"action": "post_package_sent"})
                ...
                needs_approval = False
                hybrid_ai_span.update(output={"action": "no_pending_package"})
                ...
                try:
                    ...
                    hybrid_ai_span.update(output={
                        ...
                        "routing_reasoning": routing_decision.reasoning[:100] + "..." if len(routing_decision.reasoning) > 100 else routing_decision.reasoning,
        ...
        # SPAN 5: Final Response Generation
    ...
    try:
        ...
        if error:
            ...
            return RedirectResponse(
                url=f'https://395a2c43-f9bd-494c-beb2-dd30fbdf7e7e.preview.emergentagent.com/?auth=error&message={error}&session_id={session_id}',
                status_code=302
            )
        ...
        if not code:
            ...
            return RedirectResponse(
                url=f'https://395a2c43-f9bd-494c-beb2-dd30fbdf7e7e.preview.emergentagent.com/?auth=error&message=no_code&session_id={session_id}',
                status_code=302
            )
        ...
        if result.get("authenticated", False):
            ...
            return RedirectResponse(
                url=f'https://395a2c43-f9bd-494c-beb2-dd30fbdf7e7e.preview.emergentagent.com/?auth=success&service=gmail&session_id={session_id}&timestamp={int(datetime.utcnow().timestamp())}',
                status_code=302
            )
        else:
            ...
            return RedirectResponse(
                url=f'https://395a2c43-f9bd-494c-beb2-dd30fbdf7e7e.preview.emergentagent.com/?auth=error&message=auth_failed&details={error_msg}&session_id={session_id}',
                status_code=302
            )
        ...
        # Redirect to frontend with error parameter
        return RedirectResponse(
            url=f'https://395a2c43-f9bd-494c-beb2-dd30fbdf7e7e.preview.emergentagent.com/?auth=error&message=server_error&details={str(e)}&session_id={session_id if session_id else "unknown"}',
            status_code=302
        )
