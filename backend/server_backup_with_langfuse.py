...
async def gmail_auth_callback(code: str = None, state: str = None, error: str = None, session_id: str = None):
    ...
    try:
        ...
        if result.get("authenticated", False):
            ...
            return RedirectResponse(
                url=f'https://ed44aeba-7cef-4fcd-8d35-8bddafadc1d4.preview.emergentagent.com/?auth=success&service=gmail&session_id={session_id}&timestamp={int(datetime.utcnow().timestamp())}',
                status_code=302
            )
        else:
            ...
            return RedirectResponse(
                url=f'https://ed44aeba-7cef-4fcd-8d35-8bddafadc1d4.preview.emergentagent.com/?auth=error&message=auth_failed&details={error_msg}&session_id={session_id}',
                status_code=302
            )
        ...
        # Redirect to frontend with error parameter
        return RedirectResponse(
            url=f'https://ed44aeba-7cef-4fcd-8d35-8bddafadc1d4.preview.emergentagent.com/?auth=error&message=server_error&details={str(e)}&session_id={session_id if session_id else "unknown"}',
            status_code=302
        )
